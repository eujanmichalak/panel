...
export default function SquadPage() {
  ...
  const handleAddPlayer = async () => {
    ...
    const currentTeamId = userProfile?.team_id;

    if (!trimmedName || !currentTeamId) return;

    setLoading(true);
    const { error } = await (supabase.from('players') as any).insert({
      full_name: trimmedName.toUpperCase(),
  ...
  return (
    <div className="flex flex-col gap-8 w-full pb-20 px-2 italic font-black">
      <div>
        ...
        <p className="text-slate-400 text-[10px] md:text-[12px] uppercase tracking-[0.4em] mt-2">
        ...
        <div className="flex flex-col md:flex-row gap-5 items-end">
          <div className="flex-1 w-full space-y-3">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border-2 border-slate-100 focus:border-blue-600 focus:bg-white rounded-[24px] h-[64px] px-8 text-slate-900 outline-none transition-all uppercase shadow-inner" 
            />
          </div>
          <button 
            onClick={handleAddPlayer}
            disabled={loading || !newName.trim()}
            className="w-full md:w-56 bg-blue-600 text-white rounded-[24px] shadow-xl shadow-blue-100 hover:bg-blue-700 h-[64px] uppercase tracking-widest disabled:opacity-50 transition-all active:scale-95 flex items-center justify-center gap-2"
          >
            {loading ? <Loader2 className="animate-spin" /> : <><Plus size={24} strokeWidth={4} /> DODAJ</>}
          </button>
        </div>
        {errorMsg && <div className="mt-4 text-red-500 text-xs uppercase animate-pulse flex items-center gap-2"><AlertCircle size={16}/> {errorMsg}</div>}
      </div>
        ...
        <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center bg-slate-50/50 gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              placeholder="SZUKAJ..." 
              className="w-full bg-white border-2 border-slate-200 rounded-[20px] py-3.5 pl-14 pr-6 text-sm text-slate-900 outline-none focus:border-blue-600 transition-all uppercase" 
            />
          </div>
        </div>

        <div className="p-4 md:p-8 space-y-4 min-h-[300px]">
          {filteredPlayers.length > 0 ? (
            filteredPlayers.map((player) => (
              <PlayerRow 
                key={player.id} 
                player={player} 
                onRefresh={() => userProfile?.team_id && fetchPlayers(userProfile.team_id)} 
              />
            ))
          ) : (
            <div className="flex flex-col items-center justify-center py-20 text-slate-300 gap-4 uppercase tracking-[0.3em] text-xs">Lista jest pusta</div>
          )}
        </div>
      </div>
    </div>
